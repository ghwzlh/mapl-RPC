package com.ghw.maplrpc.scanner;

import com.ghw.maplrpc.Constant.ScannerConstant;
...
public class ClassScanner {
    ...
    private static void findAndAddClassesInPackageByFile(String packageName, String packagePath, final boolean recursive, List<String> classNameList){
        ...
        File[] dirfiles = dir.listFiles(new FileFilter() {
            ...
            public boolean accept(File file) {
                return (recursive && file.isDirectory()) || (file.getName().endsWith(ScannerConstant.RPC_SCANNER_FILE_SUFFIX));
            }
        });
        //循环所有文件
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.startsWith(packageDirName)) {
                ...
                if ((idx != -1) || recursive){
                    ...
                    if (name.endsWith(ScannerConstant.RPC_SCANNER_FILE_SUFFIX) && !entry.isDirectory()) {
                        //去掉后面的".class" 获取真正的类名
    ...
    public static List<String> getClassNameList(String packageName) throws Exception{
        ...
        while (dirs.hasMoreElements()){
            ...
            //如果是以文件的形式保存在服务器上
            if (ScannerConstant.RPC_SCANNER_FILE.equals(protocol)) {
                //获取包的物理路径
                String filePath = URLDecoder.decode(url.getFile(), "UTF-8");
                //以文件的方式扫描整个包下的文件 并添加到集合中
                findAndAddClassesInPackageByFile(packageName, filePath, recursive, classNameList);
            } else if (ScannerConstant.RPC_SCANNER_JAR.equals(protocol)){
